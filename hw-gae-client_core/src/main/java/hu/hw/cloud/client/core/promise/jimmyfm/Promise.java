 ...
 * 
 */
package hu.hw.cloud.client.core.promise.jimmyfm;
 ...
 * <li>fulfilled - The state of a promise representing a successful
 * operation.</li>
...
public class Promise {
	 ...
	 * constructor even returns the created object). The resolve and reject
	 * functions, when called, resolve or reject the promise, respectively. The
	 * executor normally initiates some asynchronous work, and then, once that
	 * completes, either calls the resolve function to resolve the promise or else
	 * rejects it if an error occurred. If an error is thrown in the executor
	 * function, the promise is rejected. The return value of the executor is
	 * ignored.
	 */
	@JsFunction
	 ...
	 * or provided a non-function, the new Promise that is created simply adopts the
	 * fulfillment state of the Promise that then is called on (if it becomes
	 * fulfilled). If the second argument is omitted or provided a non-function, the
	 * new Promise that is created simply adopts the rejection state of the Promise
	 * that then is called on (if it becomes rejected).
	 *
	 * @param onFulfilled
	 ...
	 * @param onRejected
	 *            A Function called when the Promise is rejected. This function has
	 *            one argument: reason, The rejection reason.
	 * @return A Promise.
	 */
	@JsMethod(name = "catch")
	 ...
	 * iterable argument have fulfilled or rejects as soon as one of the promises in
	 * the iterable argument rejects. If the returned promise fulfills, it is
	 * fulfilled with an array of the values from the fulfilled promises in the same
	 * order as defined in the iterable. If the returned promise rejects, it is
	 * rejected with the reason from the first promise in the iterable that
	 * rejected. This method can be useful for aggregating results of multiple
	 * promises.
	 *
	 * @param promises
	 *            An iterable object (such as an Array) of promises.
	 * @return A Promise that resolves when all of the promises in the given
	 *         iterable have resolved, or rejects if any of the promises rejects.
	 */
	@JsMethod
	 ...
	 * the iterable fulfills or rejects, with the value or reason from that promise.
	 *
	 * @param promises
	 *            An iterable object, such as an Array. See iterable.
	 * @return A Promise that resolves or rejects as soon as one of the promises in
	 *         the given iterable resolves or rejects.
	 */
	@JsMethod
	...
	/**
	 * The Promise.resolve(value) method returns a Promise object that is resolved
	 * with the given value. If the value is a thenable (i.e. has a "then" method),
	 * the returned promise will "follow" that thenable, adopting its eventual
